...
files <- data.frame(path = files) %>%
  mutate(mgx_run_accession = gsub("_k21_scaled100k.csv", "", basename(path)))
...
rarecurve_results <- data.frame()
for(sample_type_i in metadata$sample_type){
  ...
  write_tsv(rarecurve_i, paste0("rarecurve_", sample_type_i, ".tsv"))
  
  # for each sample, calculate the slope of the rarefaction curve
  slope_df <- data.frame()
  for(mgx_run_accession_i in files_i$mgx_run_accession){
  ...
  rarecurve_results <- bind_rows(rarecurve_results, slope_df)
}

write_tsv(rarecurve_results, "all_rarecurveresults.tsv")